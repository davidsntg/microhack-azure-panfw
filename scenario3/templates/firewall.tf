...
resource "local_file" "bootstrap01" {
  ...
  content = templatefile("${path.module}/files/bootstrap.tpl", {
    interface-trust    = "${cidrhost(azurerm_subnet.subnet_pan_trusted.address_prefixes[0], 4)}/32"
    interface-untrust  = "${cidrhost(azurerm_subnet.subnet_pan_untrusted.address_prefixes[0], 4)}/32"
...
resource "local_file" "bootstrap02" {
  ...
  content = templatefile("${path.module}/files/bootstrap.tpl", {
    interface-trust    = "${cidrhost(azurerm_subnet.subnet_pan_trusted.address_prefixes[0], 5)}/32"
    interface-untrust  = "${cidrhost(azurerm_subnet.subnet_pan_untrusted.address_prefixes[0], 5)}/32"
    next-hop-trusted   = "${cidrhost(azurerm_subnet.subnet_pan_trusted.address_prefixes[0], 1)}"
    next-hop-untrusted = "${cidrhost(azurerm_subnet.subnet_pan_untrusted.address_prefixes[0], 1)}"
  })
